/*
	...
	* As we have to create the kth number then for each position(iterate for position from 200 to 1) iterate from 0 to 9 and get total 
	- count which satisfy the condition after assigning
	- ith pos to 0, 1, 2, ... . Now do the cummalative sum of count from 0 to 9 when cummalative_count >= K, assign that digit to the ith position
	- and go to the i-1 position(make sure to modify the sum and modulo).
